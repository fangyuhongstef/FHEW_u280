...
//  uint64_t    time_ns_new; 
  ...
  for (int i = 1; i <= 3*count; ++i) {
    ...
    if(clock_gettime(CLOCK_REALTIME, &ts) == 0) {
        std::cout << "Start Seconds:" << ts.tv_sec << std::endl;
        std::cout << "Start Nanoseconds:" << ts.tv_nsec << std::endl;
    }
    else {
    ...
    if(clock_gettime(CLOCK_REALTIME, &ts) == 0) {
        std::cout << "End Seconds:" << ts.tv_sec << std::endl;
        std::cout << "End Nanoseconds:" << ts.tv_nsec << std::endl;
    }
    else {
        perror("clock_gettime error\n");
    }
